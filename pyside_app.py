...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Initialize replay state (needed by tab builders)
        self._set_player_controls_enabled = lambda enabled: None  # Will be defined properly later
        ...
        # Create log tabs FIRST (needed by simulation tab)
        self._build_log_tabs()
        ...
        # Build and add tabs
        char_tab = self._build_character_tab()
        self.main_tabs.addTab(char_tab, "⚔  Character Setup")

        sim_tab = self._build_simulation_tab()
        self.main_tabs.addTab(sim_tab, "▶  Simulation & Results")

        # Add tab widget to root layout
        root_layout.addWidget(self.main_tabs)

        # Initialize control state based on default mode
        self._on_mode_changed()

        # ── COMPATIBILITY SECTION (keeping these for method references) ──
    ...
    def _build_characters_section(self) -> CollapsibleSection:
        """Build the Characters collapsible section with character editors."""
        char_section = CollapsibleSection("⚔  Characters")
        char_content = QVBoxLayout()

        # Character editors layout
        self.combatant_editors: list[CombatantEditor] = []
        self.editors_layout = QVBoxLayout()

        # Create initial two editors
        ...
        # Connect name changes to scenario preview
        ...
        # Connect equipment changes to action availability
        for combo in (
    ...
    def _build_scenario_section(self) -> CollapsibleSection:
        """Build the Scenario collapsible section for map configuration."""
        ...
        self.resize_map_button.setIcon(IconProvider.get_icon("expand"))
        self.resize_map_button.clicked.connect(self._resize_scenario_map)
        self.resize_map_button.setToolTip("Resize the tactical map (clears terrain)")
        ...
        self.scenario_tool_combo = QComboBox()
        self.scenario_tool_combo.addItems(["Paint Terrain", "Erase Terrain"])
        # Add "Place Character N" options (will be updated dynamically)
        for i in range(len(self.combatant_editors)):
            self.scenario_tool_combo.addItem(f"Place Character {i+1}")
        self.scenario_tool_combo.setMinimumWidth(140)
        self.scenario_tool_combo.currentIndexChanged.connect(self._on_scenario_tool_changed)
        tool_row.addWidget(self.scenario_tool_combo)
        tool_row.addStretch()
        ...
        self.scenario_terrain_combo = QComboBox()
        self.scenario_terrain_combo.addItems(["Normal", "Forest", "Water", "Mountain", "Road", "Wall"])
        self.scenario_terrain_combo.setMinimumWidth(100)
        terrain_row.addWidget(self.scenario_terrain_combo)
        ...
        self.clear_terrain_button.setToolTip("Clear all terrain from map")
        ...
        self.overlay_char_combo = QComboBox()
        self.overlay_char_combo.addItems(["None", "Character 1", "Character 2"])
        self.overlay_char_combo.setMinimumWidth(100)
        self.overlay_char_combo.currentIndexChanged.connect(self._update_scenario_overlays)
        overlay_row.addWidget(self.overlay_char_combo)
        overlay_row.addStretch()
        scenario_content.addLayout(overlay_row)

        overlay_checks_row = QHBoxLayout()
        self.overlay_range_check = QCheckBox("Range")
        self.overlay_range_check.setToolTip("Show movement range")
        self.overlay_range_check.stateChanged.connect(self._update_scenario_overlays)
        self.overlay_los_check = QCheckBox("LOS")
        self.overlay_los_check.setToolTip("Show line of sight")
        self.overlay_los_check.stateChanged.connect(self._update_scenario_overlays)
        self.overlay_path_check = QCheckBox("Path")
        self.overlay_path_check.setToolTip("Show path preview")
        self.overlay_path_check.stateChanged.connect(self._update_scenario_overlays)
        overlay_checks_row.addWidget(self.overlay_range_check)
        overlay_checks_row.addWidget(self.overlay_los_check)
        overlay_checks_row.addWidget(self.overlay_path_check)
        ...
        self.scenario_preset_combo.addItems([
            "Custom", "Duel", "Skirmish", "Siege", "2v2 Skirmish", "1v3 Ambush", "Free-for-All"
        ])
        self.scenario_preset_combo.setMinimumWidth(120)
        preset_row.addWidget(self.scenario_preset_combo)
        self.load_preset_button = QPushButton("Load")
        self.load_preset_button.setIcon(IconProvider.get_icon("refresh"))
        self.load_preset_button.clicked.connect(self._load_scenario_preset)
        self.load_preset_button.setToolTip("Load selected preset scenario")
        preset_row.addWidget(self.load_preset_button)
        preset_row.addStretch()
        scenario_content.addLayout(preset_row)

        # Editor and save/load buttons
        scenario_io_row = QHBoxLayout()
        self.scenario_editor_button = QPushButton("Editor")
        self.scenario_editor_button.setIcon(IconProvider.get_icon("edit"))
        self.scenario_editor_button.clicked.connect(self._open_scenario_editor)
        self.scenario_editor_button.setToolTip("Open full scenario editor dialog")
        ...
        self.save_scenario_button.setToolTip("Save scenario to file")
        ...
        self.load_scenario_button.setToolTip("Load scenario from file")
        scenario_io_row.addWidget(self.scenario_editor_button)
    ...
    def _build_simulation_settings_section(self) -> CollapsibleSection:
        """Build the Simulation Settings section (mode, time, surprise)."""
        settings_section = CollapsibleSection("⚙  Simulation Settings")
    ...
    def _build_run_section(self) -> CollapsibleSection:
        """Build the Run Simulation section with large, clear buttons and visible batch counts."""
        run_section = CollapsibleSection("▶  Run Simulation")
        run_content = QVBoxLayout()
        run_content.setSpacing(12)

        # Button style for prominent buttons
        button_style = """
            QPushButton {
                font-weight: bold;
                font-size: 12pt;
        ...
        """
        help_text_style = "color: #a89579; font-size: 9pt; font-style: italic; margin-bottom: 8px;"

        # ── Single Combat Button ──
        self.simulate_button = QPushButton("RUN SINGLE COMBAT")
        self.simulate_button.setIcon(IconProvider.get_icon("play"))
        self.simulate_button.setStyleSheet(button_style)
        self.simulate_button.clicked.connect(self.run_simulation)
        self.simulate_button.setToolTip("Run 1 combat with replay timeline")
        run_content.addWidget(self.simulate_button)

        help1 = QLabel("Runs 1 combat with replay timeline")
        help1.setStyleSheet(help_text_style)
        run_content.addWidget(help1)

        # Separator
        separator1 = QFrame()
        separator1.setFrameShape(QFrame.HLine)
        separator1.setFrameShadow(QFrame.Sunken)
        run_content.addWidget(separator1)
        ...
        self.batch_button.setToolTip("Run N combats and show win-rate statistics")
        run_content.addWidget(self.batch_button)

        # Batch count spinbox - ALWAYS VISIBLE!
        batch_count_row = QHBoxLayout()
        batch_count_row.addWidget(QLabel("Number of combats:"))
        self.batch_count_spin = QSpinBox()
        self.batch_count_spin.setRange(1, 10000)
        self.batch_count_spin.setValue(100)
        self.batch_count_spin.setSingleStep(50)
        self.batch_count_spin.setMinimumWidth(80)
        self.batch_count_spin.setToolTip("How many combats to run in batch simulation")
        batch_count_row.addWidget(self.batch_count_spin)
        batch_count_row.addStretch()
        run_content.addLayout(batch_count_row)

        help2 = QLabel("Runs N combats, shows win rate statistics")
        help2.setStyleSheet(help_text_style)
        run_content.addWidget(help2)

        # Separator
        separator2 = QFrame()
        separator2.setFrameShape(QFrame.HLine)
        separator2.setFrameShadow(QFrame.Sunken)
        run_content.addWidget(separator2)
        ...
        self.compare_button.setToolTip("Compare current setup vs alternative weapon")
        run_content.addWidget(self.compare_button)

        # Compare count spinbox - ALWAYS VISIBLE!
        compare_count_row = QHBoxLayout()
        compare_count_row.addWidget(QLabel("Combats per loadout:"))
        self.compare_count_spin = QSpinBox()
        self.compare_count_spin.setRange(10, 10000)
        self.compare_count_spin.setValue(100)
        self.compare_count_spin.setSingleStep(50)
        self.compare_count_spin.setMinimumWidth(80)
        self.compare_count_spin.setToolTip("How many combats to run for each loadout")
        compare_count_row.addWidget(self.compare_count_spin)
        compare_count_row.addStretch()
        run_content.addLayout(compare_count_row)

        help3 = QLabel("Compares current setup vs alternative weapon")
        help3.setStyleSheet(help_text_style)
        run_content.addWidget(help3)

        run_section.set_content_layout(run_content)
        return run_section

    def _build_player_controls_section(self) -> CollapsibleSection:
        """Build the Player Controls section (initiative, actions, movement)."""
        player_section = CollapsibleSection("🎮  Player Controls", collapsed=True)
        player_content = QVBoxLayout()
        player_content.setSpacing(8)

        # Initiative display
        initiative_row = QHBoxLayout()
        initiative_row.addWidget(QLabel("Initiative:"))
        self.initiative_label = QLabel("(run simulation)")
        ...
        player_content.addLayout(initiative_row)
        ...
        move_row.addWidget(QLabel("Move C1 to:"))
        ...
        player_section.set_content_layout(player_content)
        return player_section

    def _build_log_tabs(self) -> None:
        """Build the bottom result tabs (Action Log, Status, Map Log, Grid, Decisions)."""
    ...
    def _build_character_tab(self) -> QWidget:
        """Build the Character Setup tab with character editors and map preview."""
        tab = QWidget()
        layout = QHBoxLayout(tab)
        layout.setContentsMargins(0, 0, 0, 0)

        # Horizontal splitter: sidebar (left) + map preview (right)
        splitter = QSplitter(Qt.Horizontal)
        splitter.setHandleWidth(2)

        # Left panel: scrollable sidebar with Characters + Scenario sections
        ...
        char_section = self._build_characters_section()
        sidebar_layout.addWidget(char_section)

        # Add Scenario section
        scenario_section = self._build_scenario_section()
        ...
        # Right panel: Map preview + overlay controls
        map_container = QWidget()
        map_container.setObjectName("mapContainer")
        map_layout = QVBoxLayout(map_container)
        map_layout.setContentsMargins(8, 8, 8, 8)

        # Create the setup map widget
        self.setup_map_widget = TacticalMapWidget(
            width=self.scenario_width,
            height=self.scenario_height
        )
        ...
        overlay_controls_label = QLabel("Map Overlays:")
        overlay_controls_label.setStyleSheet("font-weight: bold; margin-top: 8px;")
        map_layout.addWidget(overlay_controls_label)

        splitter.addWidget(map_container)

        # Set initial sizes (340px sidebar, rest for map)
    ...
    def _build_simulation_tab(self) -> QWidget:
        """Build the Simulation & Results tab with clear controls and results display."""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        layout.setContentsMargins(0, 0, 0, 0)

        # Top area: horizontal splitter (controls + map)
        ...
        # Add Simulation Settings section
        settings_section = self._build_simulation_settings_section()
        controls_layout.addWidget(settings_section)

        # Add Run Simulation section
        run_section = self._build_run_section()
        controls_layout.addWidget(run_section)

        # Add Player Controls section
        player_section = self._build_player_controls_section()
        controls_layout.addWidget(player_section)

        controls_layout.addStretch()
        controls_scroll.setWidget(controls)
        top_splitter.addWidget(controls_scroll)

        # Right panel: Map + Replay
        map_panel = QWidget()
        map_panel.setObjectName("mainCanvas")
        map_panel_layout = QVBoxLayout(map_panel)
        map_panel_layout.setContentsMargins(8, 8, 8, 8)

        # Tactical map for results
        self.tactical_map_widget = TacticalMapWidget(
            width=self.scenario_width,
            height=self.scenario_height
        )
        map_panel_layout.addWidget(self.tactical_map_widget, stretch=3)

        # Replay controls below map
        replay_controls = QWidget()
        replay_layout = QHBoxLayout(replay_controls)
        replay_layout.setContentsMargins(0, 4, 0, 4)
        replay_layout.setSpacing(8)

        self.replay_prev_button = QPushButton()
        self.replay_prev_button.setIcon(IconProvider.get_icon("arrow_left"))
        self.replay_prev_button.setToolTip("Previous frame")
        self.replay_prev_button.setMaximumWidth(40)
        self.replay_prev_button.clicked.connect(self._replay_prev)

        self.replay_play_button = QPushButton()
        self.replay_play_button.setIcon(IconProvider.get_icon("play"))
        self.replay_play_button.setToolTip("Play/Pause replay")
        self.replay_play_button.setMaximumWidth(40)
        ...
        self.replay_next_button.setIcon(IconProvider.get_icon("arrow_right"))
        self.replay_next_button.setToolTip("Next frame")
        self.replay_next_button.setMaximumWidth(40)
        self.replay_next_button.clicked.connect(self._replay_next)

        self.replay_slider = QSlider(Qt.Horizontal)
        self.replay_slider.setMinimum(0)
        self.replay_slider.setMaximum(0)
        self.replay_slider.valueChanged.connect(self._on_replay_slider_moved)

        replay_layout.addWidget(QLabel("Replay:"))
        replay_layout.addWidget(self.replay_prev_button)
        replay_layout.addWidget(self.replay_play_button)
        replay_layout.addWidget(self.replay_next_button)
        replay_layout.addWidget(self.replay_slider, stretch=1)

        map_panel_layout.addWidget(replay_controls)

        top_splitter.addWidget(map_panel)

        # Set initial sizes (340px controls, rest for map)
        top_splitter.setSizes([340, 660])

        # Full vertical splitter: top (controls + map) + bottom (result tabs)
        full_splitter = QSplitter(Qt.Vertical)
        full_splitter.addWidget(top_splitter)
        full_splitter.addWidget(self.log_tabs)  # Existing bottom panel created in __init__
        full_splitter.setStretchFactor(0, 3)
        full_splitter.setStretchFactor(1, 2)
...
def main():
    ...
    # Load fantasy fonts for RPG aesthetic
    load_fantasy_fonts()
